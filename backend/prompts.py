...
DEFAULT_EVAL_PROMPT = (
    f"{BACKGROUND_PROMPT} "
    "Determine whether the study is relevant to the report's intent. Use the doctor's "
    "instructions as primary guidance. This is the first pass: respond ONLY with one "
    "of not_match, unsure, or likely_match. Never respond match in this pass. Respond "
    "ONLY as json with keys: decision (not_match|unsure|likely_match) and reason."
)
...
DEFAULT_LIKELY_GROUP_PROMPT = (
    ...
    "other studies. If none are strong, return an empty list. Respond ONLY as json with "
    "keys: very_likely_ids (array of study_id strings) and reason."
)
...
DEFAULT_LIKELY_COMPARE_PROMPT = (
    ...
    "Explain why you picked that study and not the other. Respond ONLY as json with keys: "
    "decision (match|unsure), study_id (required if match), and reason."
)
...
DEFAULT_SUMMARY_PROMPT = (
    ...
    "study matches, and suggest the creation of a new study with certain fields. "
